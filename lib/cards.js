 ...
 */
export async function createSessionCopy(character, { cardId, roomName, getRequestHeaders }) {
    const fileName = sessionFileName(cardId);
    const definition = extractDefinition(character);
    ...
    if (result?.error) throw new Error('Could not create the session copy');
