...
export class ChatBridge {
    ...
    #onStreamToken(text) {
        if (this.deps.role() !== 'host') return;
        // Coalesce full streaming snapshots to four updates per second.
        // An 80 ms timer alone exceeded the relay's six-message/second
        // sustained limit and could turn a long reply into a disconnect.
    ...
    rememberPersona(peerId, persona) {
        ...
        let previous = this.personas.get(peerId) ?? {};
        const ownerId = validIdentity(persona.ownerId) ? persona.ownerId : previous.ownerId;
        const personaId = validIdentity(persona.personaId) ? persona.personaId : previous.personaId;
        if (ownerId && personaId) {
            if (previous.personaId && (previous.ownerId !== ownerId || previous.personaId !== personaId)) previous = {};
            for (const [id, stored] of this.personas) {
                if (id !== peerId && stored.ownerId === ownerId && stored.personaId === personaId) this.personas.delete(id);
            }
        }
        const keep = (incoming, fallback) => (incoming === undefined || incoming === null ? fallback : incoming);

        this.personas.set(peerId, {
            peerId, ownerId, personaId,
            avatarId: persona.avatarId ?? previous.avatarId ?? null,
            ...
            lorebookName: persona.lorebookName
                ...
                : (persona.full ? null : previous.lorebookName ?? null),
            lorebook: persona.full ? (persona.lorebook ?? null) : persona.lorebook ?? previous.lorebook ?? null,
        ...
        // The roster ordinarily enforces this; retain a hard bound for malformed
        // or synthetic peer streams as well.
        while (this.personas.size > LIMITS.MAX_PEERS * 2) this.personas.delete(this.personas.keys().next().value);
    }

    forgetPersona(peerId) {
        if (this.personas.delete(peerId)) this.clearPersonaInjection();
    }

    retainPeers(peerIds) {
        const live = new Set(peerIds);
        for (const peerId of this.personas.keys()) if (!live.has(peerId)) this.forgetPersona(peerId);
    }
    ...
    invalidatePersonaPortrait(avatarId) {
        if (avatarId) this.#portraits.delete(avatarId);
        else this.#portraits.clear();
    }
    ...
    #localPersona() {
        ...
        return {
            ...(this.deps.personaIdentity?.(avatarId) ?? {}),
    ...
    async capturePersonaPortrait(avatarId) {
        ...
        // The cache is invalidated on persona updates, explicit resync and
        // disconnect. A same-filename image replacement must not remain cached
        // forever, but ordinary chat turns need not decode it repeatedly.
