...
export class MultiplayerUI {
    ...
    async openCardPicker() {
        ...
        const local = (context.characters ?? []).filter(character => !isRemoteCard(character));
    ...
    #renderSession() {
        ...
        list.innerHTML = entries.map(entry => {
            ...
            return `<li class="${isActive ? 'stmp-session-active' : ''}">
                ...
                <span class="flex1"></span>
                ${local >= 0
                ? `<button class="stmp-open-session menu_button" data-chid="${local}"
                        title="Open this character's chat">Open</button>`
                : '<small class="stmp-muted">not on your list yet</small>'}
            </li>`;
        }).join('');

        for (const button of list.querySelectorAll('.stmp-open-session')) {
            button.addEventListener('click', () => void this.#openSessionCard(Number(button.dataset.chid)));
        }
    }

    async #openSessionCard(chid) {
        const context = this.deps.getContext();
        try {
            await context.selectCharacterById(chid);
            // The host announces on CHAT_CHANGED; nudge it in case the chat was
            // already the open one and no event fires.
            this.session.onChatChanged();
        } catch (error) {
            this.deps.toastr.error(error?.message ?? String(error), 'Could not open that character');
        }
        this.refresh();
    }
