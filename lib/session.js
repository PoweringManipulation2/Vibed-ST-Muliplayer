...
import {
    ...
    decodeConnectionCode, encodeConnectionCode, isPrivateAddress,
} from './protocol.js';
...
export class MultiplayerSession extends EventTarget {
    ...
    async startHosting(options = {}) {
        ...
        const codeHost = advertiseHost || started.advertiseHost;
        this.code = encodeConnectionCode({ host: codeHost, port: started.port, psk });
        this.addresses = started.addresses ?? [];
        this.codeHost = codeHost;

        if (!advertiseHost && isPrivateAddress(codeHost)) {
            this.#emit('log', {
                level: 'warn',
                message: `This code points at ${codeHost}, which only works for players on your own network. `
                    + 'For anyone else, set "Address players connect to" and start again.',
            });
        }
    ...
    async join(code) {
        ...
        // content, and the relay only speaks plain ws. Failing here with an
        // explanation beats a connection that can never succeed.
        ...
        if (isPrivateAddress(target.host)) {
            this.#emit('log', {
                level: 'warn',
        ...
        await this.#openSocket(`ws://${target.host}:${target.port}/`, target.psk, { autoReconnect: true });
    }
    ...
    #reportUnreachable(detail, url) {
        ...
        const isPrivate = this.target ? isPrivateAddress(this.target.host) : false;
