...
import {
    ...
    createSessionCopy, extractDefinition, isSessionCopy, materialiseStub,
    readLocalAvatar, remoteCardId, sessionFileName, streamAvatar,
} from './cards.js';
...
export class MultiplayerSession extends EventTarget {
    ...
    async publishPersona({ force = false } = {}) {
        ...
        try {
            ...
            // SETTINGS_UPDATED drives this as well as PERSONA_UPDATED, and it fires
            // on almost any settings change — so every slider nudge re-broadcast
            // the whole persona, portrait bytes included. Enough of those exhaust
            // the relay's per-peer byte budget (LIMITS.RATE_BYTES) and the peer is
            // dropped for flooding, which reads as a random disconnect while
            // changing something unrelated.
            const fingerprint = JSON.stringify([
                persona.name, persona.description, persona.position, persona.depth,
                persona.role, persona.lorebookName, persona.avatarData?.length ?? 0,
            ]);
            if (!force && fingerprint === this._personaFingerprint) return;
            ...
            this.#emit('log', {
                level: persona.description ? 'info' : 'warn',
                message: persona.description
                    ? `Published your persona "${persona.name}"${persona.lorebookName ? ` with lorebook "${persona.lorebookName}"` : ''}`
                        + `${persona.avatarData ? ' and portrait' : ' (no portrait — is a persona selected?)'}`
                    : `Published "${persona.name}" but it has no description, so the model will only see the name.`,
            });
        } catch (error) {
    ...
    async ensureSessionCopy(cardId) {
        ...
        const findCopy = () => (context.characters ?? [])
            .findIndex(character => isSessionCopy(character) && remoteCardId(character) === cardId);

        let index = findCopy();
        if (index >= 0) return index;

        const original = (context.characters ?? []).find(character => character.avatar === entry.avatar);
        if (!original) throw new Error(`"${entry.name}" is no longer in your character list.`);

        const avatar = await createSessionCopy(original, {
            cardId,
            roomName: this.settings.roomName ?? 'this room',
            getRequestHeaders: this.deps.getRequestHeaders,
        });

        // Give the copy the original's portrait so it is recognisable in the list.
        try {
            const bytes = await readLocalAvatar(entry.avatar);
            await applyStubAvatar(avatar, bytes, { getRequestHeaders: this.deps.getRequestHeaders });
        } catch (error) {
            this.#emit('log', { level: 'warn', message: `Session copy created without its portrait: ${error.message}` });
        }

        await context.getCharacters?.();
        index = findCopy();
        if (index < 0) throw new Error('The session copy was created but could not be found.');

        this.#emit('log', {
            level: 'success',
            message: `Created a shared session copy of "${entry.name}". Your original chat is untouched.`,
        });
        return index;
    }
    ...
    async #route(payload) {
        ...
        switch (payload.op) {
            ...
            case OP.PEER_LEAVE:
                ...
                if (payload.peer?.id) this.typing.forget(payload.peer.id);
    ...
    async #onCardAvatar(payload) {
        ...
        try {
            ...
            // The refresh replaces the character objects, so anything hydrated
            // before it has to be reapplied to the new instances.
            this.hydration.dehydrateAll(this.deps.getContext().characters ?? []);
            for (const entry of this.cardIndex) {
                if (!this.pendingDefinitions.has(entry.cardId)) {
                    this.socket?.send({ op: OP.CARDS_WANT, cardId: entry.cardId, want: 'definition' });
                }
            }
            this.#emit('cards-materialised', { cards: this.cardIndex });
    ...
    #onCardDefinition(payload) {
        if (this.role !== ROLE.CLIENT) return;
        // Park it first, then try to apply. Anything that cannot be applied yet
        // is retried after the next character list refresh.
        this.pendingDefinitions.set(payload.cardId, payload.definition ?? {});
        void this.applyPendingDefinitions();
    }

    /**
     * Applies every definition whose card now exists locally.
     * Called after the definition arrives and again after each character list
     * refresh, so the race between import and delivery cannot lose one.
     */
    async applyPendingDefinitions() {
        ...
        for (const [cardId, definition] of [...this.pendingDefinitions]) {
            ...
            const character = avatar
                ? (context.characters ?? []).find(candidate => candidate.avatar === avatar)
                : (context.characters ?? []).find(candidate => remoteCardId(candidate) === cardId);
        ...
        if (missing && !this._refreshingCharacters) {
            // The card exists on disk but not yet in memory; refresh once and retry.
            this._refreshingCharacters = true;
            try {
                await context.getCharacters?.();
            } finally {
                this._refreshingCharacters = false;
            }
            if (this.pendingDefinitions.size > 0) await this.applyPendingDefinitions();
        }

        if (applied > 0) {
            this.#emit('log', {
                level: 'success',
                message: `Loaded ${applied} shared character definition${applied === 1 ? '' : 's'}.`,
            });
            this.#refreshOpenCharacter();
        }
