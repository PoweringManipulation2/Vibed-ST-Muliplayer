...
const FATAL_REASONS = new Set([
    ...
    REJECT_REASON.UNKNOWN_ROOM,
    REJECT_REASON.SERVER_CLOSING,
...
export class SecureSocket extends EventTarget {
    ...
    constructor({ url, psk, hostToken = null, autoReconnect = true }) {
        ...
        this.keys = null;
        this.sendCounter = 0n;
        this._sendBudget = new SendBudget();
    ...
    async connect() {
        ...
        socket.addEventListener('open', () => { if (this.socket === socket) void this.#startHandshake(socket); });
        ...
        socket.addEventListener('message', event => {
            ...
            this._receiveChain = this._receiveChain
                .then(() => this.socket === socket && this.#onMessage(event, socket))
                .catch(() => { /* #onMessage already reported and closed */ });
        });
        socket.addEventListener('error', () => { if (this.socket === socket) this.#emit('transport-error', { url: this.url }); });
        socket.addEventListener('close', event => { if (this.socket === socket) this.#onClose(event); });
    }
    ...
    async #startHandshake(socket) {
        ...
        try {
            ...
            socket.close(4400, 'handshake failed');
        }
    }

    async #handleChallenge(message, socket) {
    ...
    async #handleAccepted(message, socket) {
     ...
     */
    send(payload) {
        const socket = this.socket, keys = this.keys, budget = this._sendBudget;
        this._sendChain = this._sendChain.catch(() => false).then(async () => {
            if (this.state !== STATE.OPEN || !keys || this.keys !== keys || this.socket !== socket) return false;
            try {
                const counter = ++this.sendCounter;
                const frame = await sealFrame(keys, counter, payload);
                const current = () => this.socket === socket && this.keys === keys && socket.readyState === WebSocket.OPEN;
                if (!await budget.take(frame.byteLength, current)) return false;
                // Do not pile a large card transfer into an already congested TCP buffer.
                while (current() && socket.bufferedAmount > LIMITS.MAX_FRAME_BYTES) {
                ...
                if (this.socket === socket) this.#emit('error', { error, message: `Failed to send an outgoing frame: ${error.message}` });
                return false;
            }
        });
    ...
    async #onMessage(event, socket) {
        try {
            if (typeof event.data === 'string') {
                ...
                switch (message.t) {
                    case HS.CHALLENGE: return await this.#handleChallenge(message, socket);
                    case HS.ACCEPTED: return await this.#handleAccepted(message, socket);
    ...
    #onClose(event) {
        if ([4400, 4401, 4403].includes(event.code)) this.autoReconnect = false;
        if (this.openedAt && Date.now() - this.openedAt >= 60000) this.attempts = 0;
        this.#teardown();
        ...
        this.#emit('close', {
            ...
            willReconnect: !this.closedByUs && this.autoReconnect && this.attempts < 8,
        });

        if (this.closedByUs || !this.autoReconnect) return;
        if (this.attempts >= 8) {
            this.autoReconnect = false;
            this.#emit('error', { message: 'Automatic reconnect stopped after 8 unstable attempts. Check the connection and join again.' });
            return;
        }
        ...
        const delay = 500 + Math.floor(Math.random() * ceiling);
