...
export class MultiplayerSession extends EventTarget {
    ...
    async probePlugin() {
        try {
            ...
            if (data.revision !== PROTOCOL_REVISION) {
                return { ok: false, reason: `Server plugin speaks ${data.revision}, this extension speaks ${PROTOCOL_REVISION}. Update whichever is older.` };
            }
            return { ok: true, ...data };
        } catch (error) {
    ...
    async startHosting(options = {}) {
        ...
        if (!probe.ok) {
            this.#fail(`The Multiplayer server plugin is not available: ${probe.reason}`);
